...
int NetworkController::setFwmarkPolicyRule(const char *iface, int mark, bool add) {
    ...
    if(add){
	    ...
	    ret = android_fork_execvp(ARRAY_SIZE(tmp_fwmark_cmd), (char **)tmp_fwmark_cmd, NULL, false, true);
	  
	    const char *tmp_fwmark6_cmd[] = {
	        IP_PATH,
	        "-6",
	        "rule",
	        "del",
	    ...
	    ret = android_fork_execvp(ARRAY_SIZE(tmp_fwmark6_cmd), (char **)tmp_fwmark6_cmd, NULL, false, true);
	    
	    const char *tmp2_fwmark_cmd[] = {
	        IP_PATH,
	        "route",
	        "flush",
	        "table",
	    ...
	    ret = android_fork_execvp(ARRAY_SIZE(tmp2_fwmark_cmd), (char **)tmp2_fwmark_cmd, NULL, false, true);
	    const char *tmp2_fwmark6_cmd[] = {
	        IP_PATH,
	        "-6",
	        "route",
	        "flush",
	    ...
	    ret = android_fork_execvp(ARRAY_SIZE(tmp2_fwmark6_cmd), (char **)tmp2_fwmark6_cmd, NULL, false, true);
    ...
    ret = android_fork_execvp(ARRAY_SIZE(route_cmd), (char **)route_cmd, NULL, false, true);
    ...
    ret = android_fork_execvp(ARRAY_SIZE(fwmark_cmd), (char **)fwmark_cmd, NULL, false, true);
    if (ret) {
        ALOGW("skip error when set IPv4 rule and route");
        ret = 0;
    }
    ...
    ret = android_fork_execvp(ARRAY_SIZE(route6_cmd), (char **)route6_cmd, NULL, false, true);
    ...
    ret = android_fork_execvp(ARRAY_SIZE(fwmark6_cmd), (char **)fwmark6_cmd, NULL, false, true);
    if (ret) {
        ALOGW("skip error when set IPv6 rule and route");
        ret = 0;
    }
...
int NetworkController::mobileDataControl(uid_t uid, unsigned netId) const {
    Network* network;
    Permission userPermission = uid < FIRST_APPLICATION_UID ? PERMISSION_SYSTEM : PERMISSION_NONE;
    //ALOGD("mobile data control, uid %ld, netid %d", (long)uid, getDefaultNetwork());
    ...
    if (netId != NETID_UNSET)
        ...
        network = getNetworkLocked(getDefaultNetwork());

    if(!network)
        return 0;

    for(const auto& iface: network->getInterfaces())
        if(!strncmp(iface.c_str(), "cc", 2)) //celluar data check
            if(FirewallController::checkBlackListUsers((int)uid)) {
                ALOGI("uid %ld with netid %d accessing mobile network blocked", long(uid), getDefaultNetwork());
                return -EACCES;
            }
