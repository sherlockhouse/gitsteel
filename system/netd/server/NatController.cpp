...
int NatController::enableNat(const char* intIface, const char* extIface) {
    ...
    if (!strcmp(intIface, extIface)) {
        ...
        return -1;
#endif
        ALOGW("enableNat Duplicate interface specified: %s %s", intIface, extIface);
        return 0;
    }
    ...
    if(strncmp(intIface, "rndis", 5) == 0)
        PerfController::enable_Perf_RPS(intIface);
    return 0;
}
...
int NatController::disableNat(const char* intIface, const char* extIface) {
	...
	/*M: skip duplicated interfaces*/
    if (!strcmp(intIface, extIface)) {
        ALOGW("disableNat Duplicate interface specified: %s %s", intIface, extIface);
        return 0;
    }

    /*mtk_net: dump iptables before disableNat for debugging*/
    //execIptables(V4, "-t", "filter", "-L", "-v", NULL);
    execIptables(V4, "-t", "nat", "-L", "-v", NULL);
    //execIptables(V4, "-t", "mangle", "-L", "-v", NULL);
