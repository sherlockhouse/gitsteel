...
#define DBG_LOG_TAG "DBG_"LOG_TAG
...
#if MTK_BTSNOOPLOG_LIMIT_MAX_LOG_COUNT == TRUE
static void remove_oldest_file(char *folder, int max_count)
{
    DBG_LOG(LOG_TAG, "%s folder: %s, max_count: %d", __func__, folder, max_count);
    ...
    list_t * f_list_mtime_desc = list_new(osi_free);
    while ((entry = readdir(dp)) != NULL) {
        ...
        f_node * f_item = osi_malloc(sizeof(f_node));
        memset(f_item, 0, sizeof(f_node));
        sprintf(f_item->f_name, "%s/%s", folder, entry->d_name);
        struct stat file_stat;
        lstat(f_item->f_name, &file_stat);
        f_item->m_time = file_stat.st_mtim.tv_sec;
        if (list_is_empty(f_list_mtime_desc)) {
            list_append(f_list_mtime_desc, f_item);
            DBG_LOG(LOG_TAG, ">>>>%s insert 1st file: %s", __func__, f_item->f_name);
            continue;
        }
        list_node_t * node_iter = list_begin(f_list_mtime_desc);
        if (difftime(f_item->m_time,
            ((f_node*) list_node(node_iter))->m_time) < 0) {
            list_prepend(f_list_mtime_desc, f_item);
            DBG_LOG(LOG_TAG, ">>>>%s insert HEAD: %s", __func__, f_item->f_name);
            continue;
        }
        list_node_t * node_iter_next = list_next(node_iter);
        while (node_iter_next != list_end(f_list_mtime_desc)) {
            if (difftime(f_item->m_time,
                ((f_node*) list_node(node_iter_next))->m_time) < 0) {
              list_insert_after(f_list_mtime_desc, node_iter, f_item);
              DBG_LOG(LOG_TAG, ">>>>%s insert After: %s", __func__, f_item->f_name);
              break;
            }
            node_iter = node_iter_next;
            node_iter_next = list_next(node_iter_next);
        }
        if (node_iter_next == list_end(f_list_mtime_desc)) {
          list_append(f_list_mtime_desc, f_item);
          DBG_LOG(LOG_TAG, ">>>>%s append tail: %s", __func__, f_item->f_name);
        }
    }

#if DBG_LOG_ENABLE == TRUE
    DBG_LOG(LOG_TAG, "%s list length: %d.", __func__, list_length(f_list_mtime_desc));
    for (list_node_t * node_iter = list_begin(f_list_mtime_desc);
        node_iter != list_end(f_list_mtime_desc);
        node_iter = list_next(node_iter)) {
      LOG_DEBUG(LOG_TAG, "%s List File:%s", __func__, ((f_node* ) list_node(node_iter))->f_name);
    }
#endif

    int file_to_be_removed = 0;
    if ((file_to_be_removed = list_length(f_list_mtime_desc) - max_count) > 0) {
        DBG_LOG(LOG_TAG, "%s file_to_be_removed: %d", __func__, file_to_be_removed);
        for (list_node_t * node_iter = list_begin(f_list_mtime_desc);
            file_to_be_removed > 0 && node_iter != list_end(f_list_mtime_desc);
            node_iter = list_next(node_iter)) {
          remove(((f_node*) list_node(node_iter))->f_name);
          file_to_be_removed--;
          LOG_DEBUG(LOG_TAG, "%s remove btsnoop file %s", __func__,
              ((f_node*) list_node(node_iter))->f_name);
        }
    }
    list_clear(f_list_mtime_desc);
    closedir(dp);
}
#endif
...
#if MTK_BTSNOOPLOG_LIMIT_MAX_LOG_COUNT == TRUE
        int btsnoop_max_count = config_get_int(stack_config_get_interface()->get_all(),
            CONFIG_DEFAULT_SECTION, "BtSnoopMaxCount", -1);
        if (btsnoop_max_count > 0) {
          LOG_DEBUG(LOG_TAG,
              "%s check to ensure btsnoop file count is less than %d", __func__,
              btsnoop_max_count);
          char log_folder[PATH_MAX] = {0};
          strncpy(log_folder, log_path, strrchr(log_path, '/') - log_path);
          // only keep (btsnoop_max_count - 1) old btsnoop log files,
          // because we will create a new one soon
          remove_oldest_file(log_folder, btsnoop_max_count - 1);
        }
#endif
    }
    ...
    do {
        ...
        while (RW(bQHead, start) != RW(bQHead, end) ||
            ...
            while (bytesToWrite > 0) {
                ...
                if (nWritten < 0) {
                    if (errno == EINTR) {
                        LOG_DEBUG(LOG_TAG, "ignore write failed due to Interrupted system call");
