...
/* Mediatek new added vendor opcode
 *
 * A share build flag "MTK_BT_COMMON" for both vendor driver and bluedroid stack
 * to add MTK specific vendor operations.
 * Both driver and stack need to define this flag in each makefile respectively
 * if MTK_BLUEDROID_PLUS is yes.
 */
#ifdef MTK_BT_COMMON
/*  [operation]
 *      Stack call to the vendor driver to set FW assert and trigger coredump for debug.
 *  [input param]
 *      A pointer to uint32_t type indicating the assert reason.
 *  [return]
 *      0  - success.
 *      -1 - set FW assert failed, or kernel driver not ready.
 *  [callback]
 *      None.
 */
