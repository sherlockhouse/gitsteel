...
#if (MTK_HOST_SUPPORT_FW_LOG == TRUE)
#include "fw_logger.h"
#endif
...
#if (MTK_SUPPORT_FW_CORE_DUMP == TRUE)
  setFimrwareCoreDumpVendor(vendor);
#endif
...
#if (MTK_SUPPORT_FW_CORE_DUMP == TRUE)
  setFimrwareCoreDumpVendor(NULL);
#endif
...
// Interface functions
#if MTK_HCI_POWERSAVING_MODE == TRUE
...
static void transmit_command(
  ...
  // M: Avoid the timing issue that upper layer profiles want to send command
  // but HCI module is already shutted down.
  if (command_queue == NULL) {
    LOG_INFO(LOG_TAG,
        "%s Hci command queue is NULL, and can not transmit command 0x%04x",
...
static void transmit_downward(data_dispatcher_type_t type, void *data) {
  if (type == MSG_STACK_TO_HC_HCI_CMD) {
    ...
    // M:   Multithread timing issue. When jni_workqueue thread shutdown, it free packet queue and
    //      close uart port. Sometimes, workqueue thread reply packet after receive controller
    //      event before close finished.
    //      This bug is found in BT Enable + connect HS/Disale 100 autotest.
    if (packet_queue != NULL) {
        fixed_queue_enqueue(packet_queue, data);
    } else {
        LOG_WARN(LOG_TAG, "%s packet_queue is NULL !!! ignore", __func__);
    }
  }
}
...
#if MTK_SUPPORT_FW_CORE_DUMP == TRUE
  if (is_chip_doing_reset()) {
    LOG_INFO(LOG_TAG,
        "%s Bypass command_timed_out() because \"whole-chip-reset\"  or \"fw-coredump\" is triggered!",
        __func__);
    return;
  }
#endif
...
#if MTK_SUPPORT_FW_CORE_DUMP == TRUE
    triggerFirmwareAssert(BT_PANIC_HOST_ASSERT, wait_entry->opcode);
    return;
#endif
  }
...
#if (MTK_HOST_SUPPORT_FW_LOG == TRUE)
      // M: filter firmware log and save it to file
      if (filter_fw_log(incoming->buffer)) {
