...
static const tBTM_ESCO_PARAMS btm_esco_defaults =
{
    ...
    0x000c,                         /* 10 ms (HS/HF can use EV3, 2-EV3, 3-EV3)  */
#else
...
#if(MTK_HFP_eSCO_S4 == TRUE)
     BTM_ESCO_RETRANS_QUALITY        /* Retransmission Effort (Power)   */
#else
...
#if(MTK_HFP_eSCO_S4 == TRUE)
        BTM_TRACE_API("  conn_rsp   txbw 0x%x, rxbw 0x%x, lat 0x%x, voice 0x%x, retrans 0x%02x, pkt 0x%04x",
