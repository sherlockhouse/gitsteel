...
BOOLEAN BTM_PeerSupportsSecureConnections(BD_ADDR bd_addr)
{
    ...
    if ((p_dev_rec = btm_find_dev(bd_addr)) == NULL)
    {
        BTM_TRACE_DEBUG("%s: unknown BDA: %08x%04x", __FUNCTION__,
                                      ...
                                      void *p_ref_data)
{
    ...
    /* Some device reconnect with no authentication action. Do not check their security and use them as normal for temporary.*/
    if(!is_originator && interop_match_addr(INTEROP_DISABLE_SERVICE_SECURITY_CHECK,(const bt_bdaddr_t*)bd_addr))
    {
        if(p_callback)
        {
            (*p_callback)(bd_addr,transport,p_ref_data,BTM_SUCCESS);
...
static tBTM_STATUS btm_sec_dd_create_conn (tBTM_SEC_DEV_REC *p_dev_rec)
{
    ...
    {
        BTM_TRACE_DEBUG ("Security Manager: failed allocate LCB [%02x%02x%02x%02x%02x%02x]",
    ...
    {
        BTM_TRACE_DEBUG ("Security Manager: failed create  [%02x%02x%02x%02x%02x%02x]",
                            p_dev_rec->bd_addr[0], p_dev_rec->bd_addr[1], p_dev_rec->bd_addr[2],
                            p_dev_rec->bd_addr[3], p_dev_rec->bd_addr[4], p_dev_rec->bd_addr[5]);
...
void btm_sec_pin_code_request (UINT8 *p_bda)
{
    ...
    /* OR we could not allocate entry in the database reject pairing request */
             ...
             || (!p_dev_rec->is_originator
                 ...
                 &&  (p_dev_rec->dev_class[2] & BTM_COD_MINOR_KEYBOARD)) )
    {
        BTM_TRACE_DEBUG("btm_sec_pin_code_request(): Pairing disabled:%d; PIN callback:%x, Dev Rec:%x!",
