...
BOOLEAN L2CA_CancelBleConnectReq (BD_ADDR rem_bda)
{
    ...
    {
        L2CAP_TRACE_DEBUG ("L2CA_CancelBleConnectReq - different  BDA Connecting: %08x%04x  Cancel: %08x%04x",
                                            ...
                                            UINT16 latency, UINT16 timeout)
{
    ...
    if (!p_lcb || !p_acl_cb)
    {
        L2CAP_TRACE_DEBUG ("L2CA_UpdateBleConnParams - unknown BD_ADDR %08x%04x",
    ...
    {
        L2CAP_TRACE_DEBUG ("L2CA_UpdateBleConnParams - BD_ADDR %08x%04x not LE",
                              (rem_bda[0]<<24)+(rem_bda[1]<<16)+(rem_bda[2]<<8)+rem_bda[3],
                              (rem_bda[4]<<8)+rem_bda[5]);
        return(FALSE);
    }
...
BOOLEAN L2CA_EnableUpdateBleConnParams (BD_ADDR rem_bda, BOOLEAN enable)
{
    if (stack_config_get_interface()->get_pts_conn_updates_disabled())
        return false;
#if MTK_STACK_CONFIG_BL == TRUE
    if(interop_mtk_match_addr_name(INTEROP_MTK_LE_DISABLE_FAST_CONNECTION, (const bt_bdaddr_t *)rem_bda))
        return false;
#endif
    tL2C_LCB            *p_lcb;
    ...
    if (!p_lcb)
    {
        L2CAP_TRACE_DEBUG ("L2CA_EnableUpdateBleConnParams - unknown BD_ADDR %08x%04x",
            (rem_bda[0]<<24)+(rem_bda[1]<<16)+(rem_bda[2]<<8)+rem_bda[3],
            (rem_bda[4]<<8)+rem_bda[5]);
        return (FALSE);
    }
    ...
    if (p_lcb->transport != BT_TRANSPORT_LE)
    {
        L2CAP_TRACE_DEBUG ("%s - BD_ADDR %08x%04x not LE (link role %d)", __FUNCTION__,
                              (rem_bda[0]<<24)+(rem_bda[1]<<16)+(rem_bda[2]<<8)+rem_bda[3],
