                                        ...
                                        tGATTS_DATA *p_data)
{
    ...
    GATT_TRACE_EVENT("%s, conn_id: %d, request type: %d" , __func__, conn_id, type);
...
#if MTK_COMMON == TRUE
            //Add by MTK: process CCC of service changed characeristics
            if(p_data->write_req.handle == gatt_cb.gattp_attr.clt_cfg_hdl)
            {
                tGATT_PROFILE_CLCB *p_clcb = gatt_profile_find_clcb_by_conn_id(conn_id);
                if(p_clcb != NULL && p_data->write_req.len == sizeof(tGATT_CLT_CHAR_CONFIG))
                {
                    tGATT_CLT_CHAR_CONFIG *ind_value = (tGATT_CLT_CHAR_CONFIG *)p_data->write_req.value;
                    GATT_TRACE_EVENT("%s, Srv Chg Ind? %d" , __func__,*ind_value & GATT_CLT_CONFIG_INDICATION);
                    if(*ind_value & GATT_CLT_CONFIG_INDICATION)
                    {
                        tGATTS_SRV_CHG srv_chg_clt;
                        memcpy(srv_chg_clt.bda, p_clcb->bda, BD_ADDR_LEN);
                        srv_chg_clt.srv_changed = FALSE;
                        if(gatt_is_bda_in_the_srv_chg_clt_list(p_clcb->bda) != NULL || gatt_add_srv_chg_clt(&srv_chg_clt) != NULL)
                        {
                            if(btm_sec_is_a_bonded_dev(p_clcb->bda))
                            {
                                bdstr_t bdstr;
                                sprintf(bdstr, "%02x:%02x:%02x:%02x:%02x:%02x",
                                p_clcb->bda[0], p_clcb->bda[1], p_clcb->bda[2],
                                p_clcb->bda[3], p_clcb->bda[4], p_clcb->bda[5]);

                                btif_config_set_int(bdstr, "SrvChgInd", 1);
                            }
                        }
                        status = GATT_SUCCESS;
                        break;
                    ...
                    {
                        status = GATT_SUCCESS;
                        gatt_delete_dev_from_srv_chg_clt_list(p_clcb->bda);
                        if(btm_sec_is_a_bonded_dev(p_clcb->bda))
                        {
                            bdstr_t bdstr;
                            sprintf(bdstr, "%02x:%02x:%02x:%02x:%02x:%02x",
                            ...
                            btif_config_set_int(bdstr, "SrvChgInd", 0);
                        }
                        break;
                    }
                }
...
#if MTK_COMMON == TRUE
        //Add by MTK: Add CCCD for service changed characeristics
        uuid.uu.uuid16 = GATT_UUID_CHAR_CLIENT_CONFIG;
        gatt_cb.gattp_attr.clt_cfg_hdl  = GATTS_AddCharDescriptor(service_handle,(GATT_PERM_READ|GATT_PERM_WRITE),&uuid);
        GATT_TRACE_DEBUG ("gatt_profile_db_init:  handle of CCCD for service changed characteristic %d",
                       gatt_cb.gattp_attr.clt_cfg_hdl);
#endif
