...
static reactor_status_t run_reactor(reactor_t *reactor, int iterations) {
  ...
  for (int i = 0; iterations == 0 || i < iterations; ++i) {
    ...
    int ret;
    // M: add log to monitor EINTR state for debugging.
    // It says if in EINTR state, kernel should be Singal pending state.
#if 0
    OSI_NO_INTR(ret = epoll_wait(reactor->epoll_fd, events, MAX_EVENTS, -1));
#else
    while ((ret = epoll_wait(reactor->epoll_fd, events, MAX_EVENTS, -1)) == -1) {
      if (errno != EINTR) {
        break;
      }
      LOG_WARN(LOG_TAG, "%s error in epoll_wait. errno: %d, thread: %ld",
          __func__, errno, pthread_self());
    }
#endif
    if (ret == -1) {
