...
void bta_hh_le_enable(void)
{
    ...
    // MTK: le_cb_index defined only for HOGP
    //for (xx = 0; xx < BTA_HH_MAX_DEVICE; xx ++)
    for (xx = 0; xx < BTA_HH_LE_MAX_KNOWN; xx ++)
...
**
** Description      MTK:Allocate bta_hh_cb.le_cb_index
**
** Parameters:
**
*******************************************************************************/
tHID_STATUS bta_hh_le_get_le_dev_hdl(UINT8 cb_index, UINT8 *handle)
{
    UINT8   i;
    for (i = 0; i < BTA_HH_LE_MAX_KNOWN; i ++)
    {
        if(bta_hh_cb.le_cb_index[i] == cb_index)
            break;
    }

    if(i == BTA_HH_LE_MAX_KNOWN)
    {
        for( i=0; i<BTA_HH_LE_MAX_KNOWN; i++)
        {
    ...
    *handle = BTA_HH_GET_LE_DEV_HDL(i);
    return HID_SUCCESS;
}
...
void bta_hh_le_open_conn(tBTA_HH_DEV_CB *p_cb, BD_ADDR remote_bda)
{
    ...
    //MTK: use le_cb_index to maintain hid_handle for HOGP
    //p_cb->hid_handle = BTA_HH_GET_LE_DEV_HDL(p_cb->index);
    if(bta_hh_le_get_le_dev_hdl(p_cb->index, &p_cb->hid_handle) != HID_SUCCESS)
    {
...
void bta_hh_le_clear_pref_conn_params(tBTA_HH_DEV_CB *p_dev_cb)
{
    APPL_TRACE_DEBUG("%s", __func__);

    tBTM_SEC_DEV_REC  *p_dev_rec = btm_find_dev (p_dev_cb->addr);
...
#if MTK_COMMON == TRUE
        if (BTM_BLE_ISVALID_PARAM(p_cb->min_conn_int, BTM_BLE_CONN_INT_MIN, BTM_BLE_CONN_INT_MAX) &&
            BTM_BLE_ISVALID_PARAM( p_cb->max_conn_int , BTM_BLE_CONN_INT_MIN, BTM_BLE_CONN_INT_MAX) &&
            BTM_BLE_ISVALID_PARAM(p_cb->supervision_tout, BTM_BLE_CONN_SUP_TOUT_MIN, BTM_BLE_CONN_SUP_TOUT_MAX) &&
            (p_cb->slave_latency <= BTM_BLE_CONN_LATENCY_MAX || p_cb->slave_latency == BTM_BLE_CONN_PARAM_UNDEF) &&
            (p_cb->min_conn_int != BTM_BLE_CONN_PARAM_UNDEF || p_cb->max_conn_int != BTM_BLE_CONN_PARAM_UNDEF))
        {
#if MTK_STACK_CONFIG_BL == TRUE
            if(!interop_mtk_match_addr_name(INTEROP_MTK_LE_DISABLE_PREF_CONN_PARAMS, (const bt_bdaddr_t *)&p_cb->addr))
#endif
                L2CA_UpdateBleConnParams(p_cb->addr, p_cb->min_conn_int, p_cb->max_conn_int, p_cb->slave_latency, p_cb->supervision_tout);
        }
#endif
...
void bta_hh_security_cmpl(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_buf)
{
    ...
    else
    {
        ...
        else if (p_cb->conn_id != BTA_GATT_INVALID_CONN_ID)
