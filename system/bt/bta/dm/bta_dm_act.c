...
void bta_dm_add_device (tBTA_DM_MSG *p_data)
{
    ...
    if (!BTM_SecAddDevice (p_dev->bd_addr, p_dc, p_dev->bd_name, p_dev->features,
                           ...
                           p_dev->pin_length))
    {
        APPL_TRACE_EVENT("BTA_DM: Error adding device %08x%04x",
...
static void bta_dm_pinname_cback (void *p_data)
{
    ...
    if (BTA_DM_SP_CFM_REQ_EVT == event)
    {
        ...
        sec_event.cfm_req.num_val = bta_dm_cb.num_val;
        ...
        APPL_TRACE_DEBUG("%s: restore the value of cfm_req, num_val:%d, loc_auth_req:%d, \
                          rmt_auth_req:%d, just_work: %d",__func__,bta_dm_cb.num_val, \
                          btm_cb.devcb.loc_auth_req,p_dev_rec->rmt_auth_req, \
                          bta_dm_cb.just_works);
    }
    else
    {
        /* Retrieved saved device class and bd_addr */
...
void bta_dm_acl_change(tBTA_DM_MSG *p_data)
{
    ...
    if(is_new)
    {
        ...
        APPL_TRACE_EVENT("%s info: 0x%x", __func__, bta_dm_cb.device_list.peer_device[i].info);
...
static UINT8 bta_dm_ble_smp_cback (tBTM_LE_EVT event, BD_ADDR bda, tBTM_LE_EVT_DATA *p_data)
{
    ...
    switch (event)
    {
        ...
        case BTM_LE_COMPLT_EVT:
            ...
            {
                sec_event.auth_cmpl.success = TRUE;
                /* MTK: reset remove_dev_pending status to FALSE whem Bond success. */
...
void bta_dm_add_blekey (tBTA_DM_MSG *p_data)
{
    ...
    {
        APPL_TRACE_EVENT ("BTA_DM: Error adding BLE Key for device %08x%04x",
...
void bta_dm_add_ble_device (tBTA_DM_MSG *p_data)
{
    ...
    {
        APPL_TRACE_EVENT ("BTA_DM: Error adding BLE Device for device %08x%04x",
