                                                       ...
                                                       BD_ADDR bda, UINT16 handle)
{
    ...
    if (p_clreg == NULL) {
        APPL_TRACE_DEBUG("%s client_if: %d not registered bd_addr:%02x:%02x:%02x:%02x:%02x:%02x",
    ...
    APPL_TRACE_DEBUG("%s registration not found bd_addr:%02x:%02x:%02x:%02x:%02x:%02x",
        __func__, bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
