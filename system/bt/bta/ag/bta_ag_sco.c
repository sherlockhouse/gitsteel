...
#if(MTK_HFP_eSCO_S4 == TRUE)
        APPL_TRACE_EVENT("eSCO connection requests event (inx %d): rtrans %d, rxlen %d, txlen %d, txint %d",
...
#if(MTK_HFP_eSCO_S4 == TRUE)
                if (BTM_BothEndsSupportSecureConnections(p_scb->peer_addr))
                {
                     p_scb->retry_with_sco_only = FALSE;
                     APPL_TRACE_API("eSCO_S4 Setting retry_with_sco_only to FALSE");
                }
                else
                {
                     APPL_TRACE_API("Not support BR/EDR SC");
                }
#endif /* MTK_HFP_eSCO_S4 */
            }
        }
        else
        {
...
/*#if MTK_STACK_CONFIG_BL == TRUE
    if (p_scb && bta_dm_mtk_is_device_blacklisted(p_scb->peer_addr,
                stack_config_hfp_delay_open_sco_blacklist_get_interface())) {
        APPL_TRACE_DEBUG("bta_ag_sco_open: start sleep 500ms.");
        usleep(500 * 1000);
        APPL_TRACE_DEBUG("bta_ag_sco_open: start to create sco.");

#if (MTK_COMMON == TRUE)
        if (!btif_hf_is_call_idle()) {
            APPL_TRACE_DEBUG("bta_ag_sco_open: call exist, continue.");
        } else {
            APPL_TRACE_DEBUG("bta_ag_sco_open: no call exist, return. ");
            return;
        }
#endif
    }
...
#if(MTK_HFP_eSCO_S4 == TRUE)
            resp.retrans_effort = BTM_ESCO_RETRANS_QUALITY;       /* Retransmission effort                      */
#else
