...
/* Ranges of hf indicators */
/* min, max, enable */
const tBTA_HF_INDICATOR bta_hf_indicators[NUM_OF_HF_IND] =
{
    {0, 1, 1},    /*safety indicated*/
    {0, 100, 1},    /*hf_batt indicated*/
};
#endif
...
/*******************************************************************************
**
** Function         bta_ag_send_hf_ind
**
** Description      Send an indicator BIEV result code.
**
**
** Returns          void
**
...
static void bta_ag_send_hf_ind(tBTA_AG_SCB *p_scb, UINT16 id, UINT16 value)
{
    char    str[12];
    char    *p = str;
    ...
    *p++ = ',';
    utl_itoa(value, p);
    bta_ag_send_result(p_scb, BTA_AG_RES_BIND, str, 0);
...
**
*******************************************************************************/
static void bta_ag_parse_bind(char *p_s, tBTA_AG_SCB *p_scb)
{
    UINT8   Ind = 0;   /*AT+BIND= <a>,<b>,<c>,...,<n> (List HF supported indicators)*/
    int     i = 0;
    char    *p;
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        /* get integer value */
        *p = 0;
        Ind = utl_str2int(p_s); /*indicator*/
        p_s = p + 1;

        switch(Ind)
        {
            case BTA_AG_IND_SAFETY:
                /* process values */
                p_scb->safty_enabled = TRUE;
                APPL_TRACE_DEBUG("safty_enabled: %d", p_scb->safty_enabled);
                break;
            case BTA_AG_IND_HF_BATT:
                p_scb->hf_batt_enabled = TRUE;
                break;
            default:
                break;
        }
...
**
*******************************************************************************/
static INT16 bta_ag_parse_biev(char *p_s, tBTA_AG_SCB *p_scb)
{
    INT16   n[2] = {-1, -1};  /*AT+BIEV= <assigned number>,<value> (Update value of indicator)*/
    int     i;
    char    *p;
    ...
    switch(n[0])
    {
       case  BTA_AG_IND_SAFETY:
        /* process values */
        if ((n[1] < bta_hf_indicators[BTA_AG_IND_SAFETY-1].min || n[1] > bta_hf_indicators[BTA_AG_IND_SAFETY-1].max))
        {
            return -1;
        }
        p_scb->safty_ind = (BOOLEAN) n[1];
        APPL_TRACE_DEBUG("safety_ind: %d", p_scb->safty_ind);
        break;

      case BTA_AG_IND_HF_BATT:
            /* process values */
            if ((n[1] < bta_hf_indicators[BTA_AG_IND_HF_BATT-1].min || n[1] > bta_hf_indicators[BTA_AG_IND_HF_BATT-1].max))
            {
                return -1;
            }
           p_scb->hf_batt_ind = (UINT8) n[1];
            break;
        default:
            break;
    }
...
// strlcpy will not add the NULL at the end of the string as strncpy does when
// the des buffer is larger than src. So clear the memory at this point.
#if (MTK_COMMON == TRUE)
    ...
    switch (cmd)
    {
        ...
        case BTA_AG_HF_CMD_CIND:
         ...
         case BTA_AG_HF_CMD_BIND:
            if(!(p_scb->features & BTA_AG_FEAT_HF_IND))
            {
                /* don't call callback */
                event = 0;
            ...
            if (arg_type == BTA_AG_AT_TEST)
            {
                /* don't call callback */
                event = 0;
                /* send BIND string, send OK */
                bta_ag_send_result(p_scb, BTA_AG_RES_BIND, p_bta_ag_cfg->bind_info, 0);
                bta_ag_send_ok(p_scb);
            }
            if (arg_type == BTA_AG_AT_READ)
            {
                ...
                event = 0;
                /* send BIND string, send OK */
                for(i=0;i<NUM_OF_HF_IND;i++)
                bta_ag_send_hf_ind(p_scb, i+1,bta_hf_indicators[i].ag_enable);
                bta_ag_send_ok(p_scb);
            }
            break;
          ...
          case BTA_AG_HF_CMD_BIEV:
            ...
            if (arg_type == BTA_AG_AT_SET)
            {
               val.idx=bta_ag_parse_biev(p_arg, p_scb);
               switch( val.idx)
               {
                   case  BTA_AG_IND_SAFETY:
                    val.num = p_scb->safty_ind;
                    bta_ag_send_ok(p_scb);
                    break;

                   case  BTA_AG_IND_HF_BATT:
                    val.num = p_scb->hf_batt_ind;
                    bta_ag_send_ok(p_scb);
                    break;
