...
/*******************************************************************************
**
** Function         bta_av_proc_meta_cmd_ex
**
** Description      Process an AVRCP metadata command from the peer.
**
** Returns          TRUE to respond immediately
**
*******************************************************************************/
tBTA_AV_EVT bta_av_proc_meta_cmd_ex(tAVRC_RESPONSE  *p_rc_rsp, tBTA_AV_RC_MSG *p_msg, UINT8 *p_ctype)
{
    tBTA_AV_EVT evt = BTA_AV_META_MSG_EVT;
    UINT8       u8, pdu, *p;
    UINT16      u16;
    ...
    else
    {
        ...
        case AVRC_PDU_GET_CAPABILITIES:
            ...
            else
            {
                ...
                {
                    APPL_TRACE_DEBUG("Moidfy support feature name for AVRCP 1.3");
                    *p_ctype = AVRC_RSP_IMPL_STBL;
                    // disable following feature 
...
**
** Description      Process an AVRCP browse command from the peer.
**
** Returns          TRUE to respond immediately
**
*******************************************************************************/
tBTA_AV_EVT bta_av_proc_browse_cmd(tAVRC_RESPONSE *p_rsp, tBTA_AV_RC_MSG *p_msg)
{
    ...
    pdu = *(p_browse->p_browse_data);

    switch(pdu) {
        case AVRC_PDU_GET_FOLDER_ITEMS:
            scope = *(p_browse->p_browse_data + 3);
            p_rsp->get_items.pdu = AVRC_PDU_GET_FOLDER_ITEMS;
            if (!AVRC_IS_VALID_SCOPE(scope) || scope != AVRC_SCOPE_PLAYER_LIST) {
                evt = 0;
                p_rsp->get_items.status = AVRC_STS_BAD_SCOPE;
                APPL_TRACE_ERROR("Not support this scope: x%x", scope);
            } else if (scope == AVRC_SCOPE_PLAYER_LIST) {
                p = p_browse->p_browse_data + 1;
                BE_STREAM_TO_UINT16(u16,p);

                p = p_browse->p_browse_data + 4;
                UINT32 start, end;
                BE_STREAM_TO_UINT32(start,p);
                BE_STREAM_TO_UINT32(end,p);

                if (end < start) {
                    evt = 0;
                    p_rsp->get_items.status = AVRC_STS_INTERNAL_ERR;
                    APPL_TRACE_ERROR("end index x%x is smaller than start index x%x", end, start);
                }
            }
            break;
        case AVRC_PDU_GET_TOTAL_NUMBER_OF_ITEMS:
            scope = *(p_browse->p_browse_data + 3);
            p_rsp->total_num.pdu = AVRC_PDU_GET_TOTAL_NUMBER_OF_ITEMS;
            if (!AVRC_IS_VALID_SCOPE(scope) || scope != AVRC_SCOPE_PLAYER_LIST) {
                evt = 0;
                p_rsp->total_num.status = AVRC_STS_BAD_SCOPE;
                APPL_TRACE_ERROR("Not support this scope: x%x", scope);
            } else if (scope == AVRC_SCOPE_PLAYER_LIST) {
                p = p_browse->p_browse_data + 1;
                BE_STREAM_TO_UINT16(u16,p);
                if (u16 != 1 || p_browse->browse_len != 4) {
                    evt = 0;
                    p_rsp->get_items.status = AVRC_STS_INTERNAL_ERR;
                    APPL_TRACE_ERROR("Wrong length: x%x", u16);
                }
            }
            break;
        default:
            evt = 0;
            p_rsp->rsp.pdu = AVRC_PDU_GENERAL_REJECT;
            p_rsp->rsp.status = AVRC_STS_BAD_CMD;
            APPL_TRACE_ERROR("Not support this cmd: x%x", pdu);
            break;
    }
...
void bta_av_rc_msg(tBTA_AV_CB *p_cb, tBTA_AV_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_av_rc_msg opcode: %d", p_data->rc_msg.opcode);
    ...
    else if (p_data->rc_msg.opcode == AVRC_OP_VENDOR)
    {
        ...
        if (p_cb->features & BTA_AV_FEAT_BROWSE) {
            ...
            if (evt != 0) {
                memset(&av.meta_msg, 0, sizeof(tBTA_AV_META_MSG));
