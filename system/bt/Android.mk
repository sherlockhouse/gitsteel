...
# Add new feature option (MTK_BT_HOST_SNOOP_LOG_SUPPORT) to control BT host snoop log
$(info MTK_BT_STACK - MTK_BT_HOST_SNOOP_LOG_SUPPORT: $(MTK_BT_HOST_SNOOP_LOG_SUPPORT))
ifeq ($(strip $(MTK_INTERNAL)), yes) # mtk internal build
  mdroid_CFLAGS += \
    -DMTK_BTSNOOPLOG_THREAD=TRUE \
    -DMTK_HOST_SUPPORT_FW_LOG=TRUE
    MTK_BT_LOG_MODE_CUSTOMIZED = yes
else # customer build
  ifneq ($(strip $(TARGET_BUILD_VARIANT)), user) # customer non-user build
    mdroid_CFLAGS += \
      -DMTK_BTSNOOPLOG_THREAD=TRUE \
      -DMTK_HOST_SUPPORT_FW_LOG=TRUE
    MTK_BT_LOG_MODE_CUSTOMIZED = yes
  else # customer user build
    ifeq ($(strip $(MTK_LOG_CUSTOMER_SUPPORT)), yes)
      ifeq ($(strip $(MTK_BT_HOST_SNOOP_LOG_SUPPORT)), yes) # forcedly enabled by customer
        mdroid_CFLAGS += \
          -DMTK_BTSNOOPLOG_THREAD=TRUE \
          -DMTK_HOST_SUPPORT_FW_LOG=TRUE
        MTK_BT_LOG_MODE_CUSTOMIZED = yes
        ...
        mdroid_CFLAGS += \
          -DMTK_BTSNOOPLOG_THREAD=FALSE \
          -DMTK_HOST_SUPPORT_FW_LOG=FALSE
        MTK_BT_LOG_MODE_CUSTOMIZED = no
      endif
    endif
    mdroid_CFLAGS += -DMTK_BTSNOOPLOG_LIMIT_MAX_LOG_COUNT=FALSE
  endif #end of customer build
endif #end of ifeq ($(strip $(MTK_INTERNAL)), yes)
$(info MTK_BT_STACK - MTK_BT_LOG_MODE_CUSTOMIZED: $(MTK_BT_LOG_MODE_CUSTOMIZED))
$(info MTK_BT_STACK - mdroid_CFLAGS: $(mdroid_CFLAGS))
