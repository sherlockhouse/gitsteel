    ...
    for (i = 0; i < BTIF_HH_MAX_HID; i++) {
        ...
        if (p_dev->dev_status != BTHH_CONN_STATE_UNKNOWN &&
            ...
            APPL_TRACE_WARNING("%s: Found an existing device with the same handle "
                                                                ...
                                                                p_dev->dev_status);
            APPL_TRACE_DEBUG("%s:     bd_addr = [%02X:%02X:%02X:%02X:%02X:]", __FUNCTION__,
                 p_dev->bd_addr.address[0], p_dev->bd_addr.address[1], p_dev->bd_addr.address[2],
                 p_dev->bd_addr.address[3], p_dev->bd_addr.address[4]);
                 APPL_TRACE_DEBUG("%s:     attr_mask = 0x%04x, sub_class = 0x%02x, app_id = %d",
                             ...
                             int dscp_len, UINT8 *p_dscp)
{
    ...
    APPL_TRACE_DEBUG("%s: fd = %d, name = [%s], dscp_len = %d", __FUNCTION__,
                                                                    p_dev->fd, dev_name, dscp_len);
    APPL_TRACE_DEBUG("%s: vendor_id = 0x%04x, product_id = 0x%04x, version= 0x%04x,"
