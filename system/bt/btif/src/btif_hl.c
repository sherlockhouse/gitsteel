...

{
    ...
    BTIF_TRACE_DEBUG("%s de-reg status=%d app_handle=%d app_id=%d", __FUNCTION__,
                p_data->dereg_cfm.status, p_data->dereg_cfm.app_handle, p_data->dereg_cfm.app_id);
...
static void btif_hl_proc_cb_evt(UINT16 event, char* p_param){
    ...
    switch (event)
    {
        ...
        case BTIF_HL_UNREG_APP:
            ...
            BTIF_TRACE_DEBUG("Rcv BTIF_HL_UNREG_APP app_idx=%d app_id=%d", p_data->unreg.app_idx, p_acb->app_id);
...
static void btif_hl_upstreams_evt(UINT16 event, char* p_param){
    ...
    switch (event)
    {
        ...
        case BTA_HL_DEREGISTER_CFM_EVT:
            ...
            BTIF_TRACE_DEBUG("app_handle=%d status=%d app_id=%d",
                              p_data->dereg_cfm.app_handle,
                              p_data->dereg_cfm.status,
                              p_data->dereg_cfm.app_id);
