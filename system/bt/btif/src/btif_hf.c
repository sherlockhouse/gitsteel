...
/*******************************************************************************
**
** Function         process_hf_ind_evt
**
** Description      Switches context from BTE to BTIF for all HF indicator events
**
** Returns          void
**
*******************************************************************************/
 static void process_hf_ind_evt(tBTA_AG_VAL *val, int idx)
{
    switch (val->idx)
       {
           case BTA_AG_IND_SAFETY:
               HAL_CBACK(bt_hf_callbacks, biev_safety_cb, val->num, &btif_hf_cb[idx].connected_bda);
               BTIF_TRACE_DEBUG("biev_safety_cb %d,%d,%s ",val->idx, val->num, &btif_hf_cb[idx].connected_bda);
               break;

           case BTA_AG_IND_HF_BATT:
               HAL_CBACK(bt_hf_callbacks, biev_battery_level_cb, val->num, &btif_hf_cb[idx].connected_bda);
               break;
           default:
               break;
        }
...
static void btif_hf_upstreams_evt(UINT16 event, char* p_param)
{
    ...
    switch (event)
    {
        ...
        case BTA_AG_AT_BIEV_EVT:
            BTIF_TRACE_DEBUG("AT+BIEV is %d,%d ", p_data->val.idx, p_data->val.num);
            process_hf_ind_evt(&p_data->val, idx);
            break;
#endif
