...
static void btif_update_remote_version_property(bt_bdaddr_t *p_bd)
{
    ...
    BTIF_TRACE_DEBUG( "%s: remote version info [%s]: %x, %x, %x", __func__,bdaddr_to_string(p_bd, bdstr, sizeof(bdstr)),
...
static void btif_dm_auth_cmpl_evt (tBTA_DM_AUTH_CMPL *p_auth_cmpl)
{
    ...
    if (bdcmp(p_auth_cmpl->bd_addr, pairing_cb.bd_addr) != 0) {
      ...
      BTIF_TRACE_EVENT("%s skipping SDP since we did not initiate pairing to %s.", __func__, address);
      return;
    }
...
static void btif_dm_search_services_evt(UINT16 event, char *p_param)
{
    ...
    switch (event)
    {
        case BTA_DM_DISC_RES_EVT:
        {
            ...
            {
                BTIF_TRACE_WARNING("%s:SDP failed after bonding re-attempting when BTA_DM_DISC_CMPL_EVT is received", __FUNCTION__);
                return;
            }
            prop.type = BT_PROPERTY_UUIDS;
            prop.len = 0;
...
void btif_dm_save_ble_bonding_keys(void)
{
    ...
    //Add for ble device pair on guest mode
...
#if (MTK_COMMON == TRUE)
        char host_name[MAX_VALUE_LEN];
        //*/ freeme.biantao, 20170113. Altered product model.
        osi_property_get(PROPERTY_PRODUCT_MODEL, host_name, BTM_DEF_LOCAL_NAME);
        osi_property_get("persist.freeme.product.model", host_name, host_name);
        int result = custom_get_string(MODULE_BLUETOOTH, HOST_NAME, host_name, host_name);
        /*/
        int result = custom_get_string(MODULE_BLUETOOTH, HOST_NAME, host_name, BTM_DEF_LOCAL_NAME);
        //*/

        if(result > 0)
        {
            strncpy(btif_default_local_name, host_name, max_len);
        }
#else
