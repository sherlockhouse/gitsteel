...
#endif
#if MTK_STACK_BA2BD_CONVERT == TRUE
...
bool btif_get_device_type(const BD_ADDR bd_addr, int *p_device_type)
{
    ...
    BTIF_TRACE_DEBUG("%s: Device [%s] type %d", __FUNCTION__, bd_addr_str, *p_device_type);
    return TRUE;
}
...
bool btif_get_address_type(const BD_ADDR bd_addr, int *p_addr_type)
{
    ...
    BTIF_TRACE_DEBUG("%s: Device [%s] address type %d", __FUNCTION__, bd_addr_str, *p_addr_type);
    return TRUE;
}
