...
void btif_av_event_deep_copy(UINT16 event, char *p_dest, char *p_src)
{
    ...
    switch (event)
    {
        ...
        case BTA_AV_BROWSE_MSG_EVT:
            if (av_src->meta_msg.p_data && av_src->meta_msg.len)
            {
                BTIF_TRACE_DEBUG("[Alice] browse have p_data?");
                av_dest->meta_msg.p_data = osi_calloc(av_src->meta_msg.len);
            ...
            if (av_src->meta_msg.p_msg)
            {
                ...
                if (av_src->meta_msg.p_msg->browse.p_browse_data &&
                    av_src->meta_msg.p_msg->browse.browse_len)
                {
                    av_dest->meta_msg.p_msg->browse.p_browse_data = osi_calloc(
                        av_src->meta_msg.p_msg->browse.browse_len);
                    assert(av_dest->meta_msg.p_msg->browse.p_browse_data);
                    memcpy(av_dest->meta_msg.p_msg->browse.p_browse_data,
                        av_src->meta_msg.p_msg->browse.p_browse_data,
                        av_src->meta_msg.p_msg->browse.browse_len);
                }
            }
            break;
        ...
        case BTA_AV_BROWSE_MSG_EVT:
            {
                ...
                if (av->meta_msg.p_msg)
                {
                    if (av->meta_msg.p_msg->browse.p_browse_data)
