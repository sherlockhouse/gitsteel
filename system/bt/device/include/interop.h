...
typedef enum {
  ...
  // their reconnect.
  INTEROP_DISABLE_SERVICE_SECURITY_CHECK,
  ...
  //Some device like ELECOM cann't handle the peripheral pereferred connection parameters
  //update procedure
 ...
 //Some devices cannot work normal as a master if the connection is initiated by them self
 ...
 //hen HFP set fail, so extend the timer to reduce the conllision
 ...
 //Some device cannot work normal as a slave if the connection is established by themself.
  ...
  INTEROP_MTK_PRIOR_ACCEPT_AVDTP_CONN,
