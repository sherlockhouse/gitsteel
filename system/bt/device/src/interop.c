...
bool interop_match_addr(const interop_feature_t feature, const bt_bdaddr_t *addr) {
  ...
  if (interop_match_fixed_(feature, addr) || interop_match_dynamic_(feature, addr)) {
    LOG_WARN(LOG_TAG, "%s() Device match for interop workaround %s.",
          __func__, interop_feature_string_(feature));
    return true;
  }
...
static const char* interop_feature_string_(const interop_feature_t feature) {
  switch (feature) {
    ...
    CASE_RETURN_STR(INTEROP_DISABLE_SERVICE_SECURITY_CHECK)
    ...
    CASE_RETURN_STR(INTEROP_MTK_PRIOR_ACCEPT_AVDTP_CONN)
#endif
  }
