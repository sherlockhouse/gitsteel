...
{
  char cryptfs_reboot_delay[PROPERTY_VALUE_MAX];
  property_get("vold.cryptfs_reboot.delay", cryptfs_reboot_delay, "0");

  int reboot_delay =  atoi(cryptfs_reboot_delay);
  if (reboot_delay > 0 ) {
    SLOGI("%s: try to sleep %ds", __FUNCTION__, reboot_delay);
    sleep(reboot_delay);
  }
...
static int create_crypto_blk_dev(struct crypt_mnt_ftr *crypt_ftr,
  ...
  #define DM_DEV_EXIST_RETRIES 20
  int i = 0;
  for (i = 0; i < DM_DEV_EXIST_RETRIES; i++) {
      if (!(access(crypto_blk_name, F_OK))) {
          break;
      }
      usleep(100 * 1000);
  }

  if (i == DM_DEV_EXIST_RETRIES) {
      SLOGE("Cannot find the dm-crypt device created, %s", crypto_blk_name);
  }
  else {
      SLOGI("The dm-crypt device is already created, %s", crypto_blk_name);
  }
...
static int delete_crypto_blk_dev(char *name)
{
  ...
  for (i = 0; i < DM_DEV_REMOVE_RETRIES; i++) {
    if (!(retval = ioctl(fd, DM_DEV_REMOVE, io))) {
        break;
    }else{
        SLOGE("Cannot remove dm-crypt device, retry(%d), rtn(%d)\n", i , retval);
        if (retval == -1 && errno == ENXIO) {
            SLOGI("device doesn't appear to be in the dev hash table.");
            break;
        }
    }
    usleep(500000);
