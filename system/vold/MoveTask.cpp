...
static status_t execRm(const std::string& path, int startProgress, int stepProgress) {
    ...
    while (true) {
        ...
        notifyProgress(startProgress + CONSTRAIN(expectedBytes == 0 ? 0 :(int)
...
static status_t execCp(const std::string& fromPath, const std::string& toPath,
    ...
    LOG(DEBUG) << "expectedBytes =  " << expectedBytes;
    LOG(DEBUG) << "startFreeBytes = " << startFreeBytes;
    ...
    while (true) {
        ...
        LOG(DEBUG) << "deltaFreeBytes = " << deltaFreeBytes;
        notifyProgress(startProgress + CONSTRAIN(expectedBytes == 0 ? 0 :(int)
                ((deltaFreeBytes * stepProgress) / expectedBytes), 0, stepProgress));
    }
    return -1;
...
static void bringOnline(const std::shared_ptr<VolumeBase>& vol) {
    if (vol->getState() != VolumeBase::State::kBadRemoval &&
        vol->getState() != VolumeBase::State::kRemoved) {
        vol->destroy();
    }
    vol->setSilent(false);
    vol->create();
