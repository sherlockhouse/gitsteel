...
int CommandListener::VolumeCmd::runCommand(SocketClient *cli,
    ...
    if (cmd == "reset") {
        ...
        /* if receiving 'reset' command with small serial number again,
           it should be caused by framework unexpected reboot (fatal error).
           And vold will skip umounting operation to keep the storage usable,
           or some logger will stop and lose log. */
        if (isFirstResetCommand == false && cli->getCmdNum() < 8) {
            vm->mSkipUmountForResetCommand = true;
        }
        SLOGI("%s: isFirstResetCommand=%d, mSkipUmountForResetCommand=%d", __FUNCTION__, isFirstResetCommand, vm->mSkipUmountForResetCommand);
        int cond =  vm->reset();

        vm->mSkipUmountForResetCommand = false;
        if (isFirstResetCommand) {
            ...
            if (!strcmp(crypto_state, "unencrypted") || !strcmp(crypto_type, "file") || !strcmp(decrypt_state, "trigger_restart_framework") ) {
              isFirstResetCommand = false;
            }
        }
        return sendGenericOkFail(cli, cond);
...
static android::status_t mountInNamespace(uid_t uid, int device_fd, const std::string& path) {
    ...
    SLOGD("@@mountInNamespace ForceUnmount --");
...
*/
    for (int i = 0; i < 5; i++) {
        if (mount("/dev/fuse", path.c_str(), "fuse",
            MS_NOSUID | MS_NODEV | MS_NOEXEC | MS_NOATIME, opts.c_str()) == 0) {
            SLOGI("%s sucessfully mounted ", opts.c_str());
            return android::OK;
        }
        sleep(1);
        SLOGE("mount fail %d time, %s (%s)", i, opts.c_str(), strerror(errno));
    }
    SLOGE("Giving up on mount %s (%s)", opts.c_str(), strerror(errno));
    return -errno;

    SLOGD("@@mountInNamespace --");
...
static android::status_t runCommandInNamespace(const std::string& command,
    ...
    SLOGD("Run app fuse command %s,path %s,namespace %d",command.c_str(),path.c_str(),uid);
