...
void VolumeManager::handleBlockEvent(NetlinkEvent *evt) {
    ...
    if (mDebug) {
        ...
        //LOG(VERBOSE) << "----------------";
        if (dp) {
           LOG(VERBOSE) << "handleBlockEvent with action " << (int) evt->getAction() << ", DEVPATH: " << dp;
        }
        if (dp && ( !strstr(dp, "virtual/block/loop") && !strstr(dp, "mmc_host/mmc0"))) {
          evt->dump();
        }
    }
    ...
    case NetlinkEvent::Action::kChange: {
        ...
        for (auto disk : mDisks) {
            if (disk->getDevice() == device) {
                if (disk->mSkipDiskChangedUevent) {
                    LOG(DEBUG) << "Skip this disk:" << major << "," << minor << " changed uevent";
                    disk->mSkipDiskChangedUevent = false;
                }
                else {
                    if (disk->getJustPartitioned() || evt->findParam("DISK_MEDIA_CHANGE")) {
                        LOG(DEBUG) << "set mSkipDiskChangedUevent to true for the disk:" << major << "," << minor;
                        disk->mSkipDiskChangedUevent = true;
                    }
                    disk->readMetadata();
                    disk->readPartitions();
                }
...
int VolumeManager::mkdirs(char* path) {
    ...
    if (strncmp(path, "/storage/", 9) == 0) {
        bool isStorageMounted = false;

        if (mInternalEmulated != nullptr && mInternalEmulated->getState() == android::vold::VolumeBase::State::kMounted
            && strstr(path, mInternalEmulated->getPath().c_str())) {
            isStorageMounted = true;
        }
        if (!isStorageMounted) {
            std::list<std::string> emulatedIds;
            listVolumes(android::vold::VolumeBase::Type::kEmulated, emulatedIds);
            for (auto id : emulatedIds) {
                  auto vol = findVolume(id);
                  if (vol != nullptr && vol->getState() == android::vold::VolumeBase::State::kMounted
                      && strstr(path, vol->getPath().c_str())) {
                      isStorageMounted = true;
                      break;
                  }
            }
        ...
        if (!isStorageMounted) {
              ...
              listVolumes(android::vold::VolumeBase::Type::kPublic, publicIds);
              for (auto id : publicIds) {
                  auto vol = findVolume(id);
                  if (vol != nullptr && vol->getState() == android::vold::VolumeBase::State::kMounted
                      && strstr(path, vol->getPath().c_str())) {
                      isStorageMounted = true;
...
void VolumeManager::mountallVolumes() {
   ...
   for (auto id : privateIds) {
       auto vol = findVolume(id);
       SLOGI("%s: privateIds: %s", __FUNCTION__, id.c_str());
       if (vol != nullptr && vol->getState() != android::vold::VolumeBase::State::kMounted) {
           vol->mount();
       }
   }
    ...
    for (auto id : emulatedIds) {
        auto vol = findVolume(id);
        SLOGI("%s: emulatedIds: %s", __FUNCTION__, id.c_str());
        if (vol != nullptr && vol->getState() != android::vold::VolumeBase::State::kMounted) {
            vol->mount();
            primary = vol;
        }
    ...
    for (auto id : publicIds) {
        auto vol = findVolume(id);
        SLOGI("%s: publicIds: %s", __FUNCTION__, id.c_str());
        if (vol != nullptr && vol->getState() != android::vold::VolumeBase::State::kMounted) {
            int mountFlags = vol->getMountFlags();
            auto disk = findDisk(vol->getDiskId());
            if (disk != nullptr && (disk->getFlags() & android::vold::Disk::Flags::kAdoptable)) {
    ...
    if ((fp = fopen(MEMINFO_PATH, "r"))) {
        ...
        if (fgets(line, sizeof(line), fp)) {
            if (sscanf(line, "MemTotal: %d kB", &memTotalSize) == 1 && memTotalSize != 0) {
                ...
                SLOGD("dirtyRatioVal = %d", dirtyRatioVal);
            }
        } else
...
int VolumeManager::shareVolume(const std::string& id, const char *method) {
    auto vol = findVolume(id);
...
int VolumeManager::unshareVolume(const std::string& id, const char *method) {
    auto vol = findVolume(id);
...
int VolumeManager::shareEnabled(const std::string& id, const char *method, bool *enabled) {
    auto vol = findVolume(id);
    ...
    if (vol->getState() != android::vold::VolumeBase::State::kShared) {
        *enabled = false;
    } else {
        *enabled = true;
    }
...
int VolumeManager::ipo(const char *cmd) {
    if (!strcmp(cmd, "startup")) {
        ...
        SLOGE("Unsupported ipo command, %s", cmd);
    }
    return 0;
}
